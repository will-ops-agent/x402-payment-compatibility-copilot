...
function analyzePreflight(input: z.infer<typeof preflightInputSchema>) {
  ...
  const verdict = riskScore >= 70 ? "high-risk" : riskScore >= 35 ? "needs-hardening" : "compatible";
  ...
  const remediationSteps = Array.from(
    new Set([
  ...
  return {
    ...
    localInvokeExamples: {
      freeEntrypoint: {
        endpoint: "/api/preflight-x402-free/invoke",
      },
      paidEntrypoint: {
        endpoint: "/api/preflight-x402/invoke",
      },
      directRoute: {
...
api.post("/preflight-x402-free/invoke", async (c) => {
...
api.post("/preflight-x402/invoke", async (c) => {
  const body = await c.req.json().catch(() => null);
  const parsed = invokeEnvelopeSchema.safeParse(body);
...
api.post("/preflight-x402", async (c) => {
